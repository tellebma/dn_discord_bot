...
async function creerStatsGenerales(gestionnaire: GestionnaireStatistiques): Promise<EmbedBuilder> {
  ...
  if (stats.topJeux.length > 0) {
    const topJeux = stats.topJeux.map((j, i) =>
 ...
 */
async function creerStatsTendances(gestionnaire: GestionnaireStatistiques): Promise<EmbedBuilder[]> {
  ...
  for (const tendance of tendances) {
    ...
    const jeuxText = tendance.jeux.slice(0, 10).map((jeu, i) => {
 ...
 */
async function creerStatsUtilisateurs(gestionnaire: GestionnaireStatistiques): Promise<EmbedBuilder> {
  const stats = gestionnaire.obtenirStatsGenerales();
